...
## Research films (Video/research)

- Published films: `v4/` (four cards) and `v3/cherry-action-30s` (cherry card), wired in
  `index.html` and checked by `tests/research-captions.cjs`.
- **v5 remake in progress** (`Video/research/v5/`): five films, shared cast, live-action with
  Veo native audio, generated in Google Flow. `films.json` is the single source of truth;
  `storyboard.html` and `PROMPT_PACK.md` are generated by `node scripts/build-v5-storyboard.cjs`
  (do not hand-edit them). `scripts/assemble-v5.cjs` builds the 30 s MP4/JPG/VTT once the
  Flow clips exist. `index.html` is not switched to v5 until the finished films are approved.
- **Cross-verification with Astra (owner's rule).** The owner works with a second agent, Astra
  (computer use, Blender, Google Flow operation; it reviewed the v3 cuts, see
  `v3/edit-plan.json`). Any new film idea, script or production plan must be cross-checked with
  Astra at the idea stage. Astra is not reachable from Claude sessions, so the exchange goes
  through the repo: write `ASTRA_REVIEW_REQUEST.md` in the version folder, mark the concept as
  awaiting review, and fold the answers (`ASTRA_REVIEW.md`) back into the plan. Where the two
  agents disagree, the owner decides.
